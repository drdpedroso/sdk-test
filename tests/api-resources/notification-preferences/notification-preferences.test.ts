...
describe('resource notificationPreferences', () => {
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.notificationPreferences.retrieve('in_app', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(HeadlessClientSDK.NotFoundError);
  });

  test('update: only required params', async () => {
    const responsePromise = client.notificationPreferences.update('in_app', {
      enabled: true,
      type: 'post_comment',
    });
    const rawResponse = await responsePromise.asResponse();
  ...
  test('update: required and optional params', async () => {
    const response = await client.notificationPreferences.update('in_app', {
      enabled: true,
      type: 'post_comment',
    });
  });
