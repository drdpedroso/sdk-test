...
describe('resource reactions', () => {
  test('create: only required params', async () => {
    const responsePromise = client.reactions.create({ chat_room_message: 0, emoji: 'emoji' });
  ...
  test('create: required and optional params', async () => {
    const response = await client.reactions.create({ chat_room_message: 0, emoji: 'emoji' });
  });
});
