...
describe('resource chatRoomParticipants', () => {
  ...
  test('update: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.chatRooms.chatRoomParticipants.update('chat_room_uuid', 'id', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('update: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.chatRooms.chatRoomParticipants.update('chat_room_uuid', 'id', { admin: true, archived: true, muted: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(HeadlessClientSDK.NotFoundError);
  });
});
