...
export class Spaces extends APIResource {
   ...
   */
  update(notificationPreferenceMedium: 'in_app' | 'push', id: number, params: SpaceUpdateParams, options?: Core.RequestOptions): Core.APIPromise<MediumSpaceNotificationPreferencesUpdate> {
    const { choice } = params;
    return this._client.put(`/api/headless/v1/notification_preferences/${notificationPreferenceMedium}/spaces/${id}`, { query: { choice }, ...options });
  }
}
