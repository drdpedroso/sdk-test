...
export class Spaces extends APIResource {
   ...
   */
  list(communityMemberId: number, query?: SpaceListParams, options?: Core.RequestOptions): Core.APIPromise<PaginatedSpaces>
  list(communityMemberId: number, options?: Core.RequestOptions): Core.APIPromise<PaginatedSpaces>
  list(communityMemberId: number, query: SpaceListParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<PaginatedSpaces> {
    if (isRequestOptions(query)) {
      return this.list(communityMemberId, {}, query);
    }
    return this._client.get(`/api/headless/v1/community_members/${communityMemberId}/spaces`, { query, ...options });
  }
}
